...
public:
    ...
    template<typename C = std::remove_const<T>, std::enable_if_t<std::is_same_v<T, std::add_const_t<C>>, bool> = true>
    constexpr MemoryView(const MemoryView<C> &other) : begin_(other.begin()), size_(other.size()) {}

    template<std::size_t N>
    constexpr MemoryView(std::array<T, N> &ar) : begin_(ar.begin()), size_(ar.size()) {}
