...
FreeTypeGlyph FreeTypeFace::renderGlyph(const std::uint32_t symbol, const int loadFlags,
                                        const FT_Render_Mode renderMode) const
{
    if (const auto errorCode = FT_Load_Char(ftFace.get(), symbol, loadFlags); errorCode != 0)
    {
        throw std::runtime_error(
            std::string("Failed to load FreeType face char code ") + std::to_string(symbol) + ": " +
            FT_Error_String(errorCode));
    }
    if (const auto errorCode = FT_Render_Glyph(ftFace->glyph, renderMode); errorCode != 0)
    {
        throw std::runtime_error(
            std::string("Failed to render FreeType face char code ") + std::to_string(symbol) + ": " +
            FT_Error_String(errorCode));
    }
